...
class RPN_FUSION(nn.Module):
    ...
    def forward(self, x, f_view, idxs_norm, bev=None):
        ...
        rgb_w0 = self.rgb_refine[0].weight
        rgb_w1 = self.rgb_refine[3].weight
        rgb_gate = self.crop_gate.spatial_basic.weight

        # bev_vis = bev_gated_s1[0,:64,...].view(64,1,200,176)
        # rgb_vis = rgb_gated_s1[0,:64,...].view(64,1,200,176)
        bev_vis = bev_gated_s1[:,:64,...].permute(1,0,2,3)
        rgb_vis = rgb_gated_s1[:,:64,...].permute(1,0,2,3)
        ret_dict['gated_concat_feat'] = concat_feat.permute(1,0,2,3)
        ret_dict['gated_bev_feat'] = bev_feature.permute(1,0,2,3)
        ret_dict['bev_vis'] = bev_vis
        ret_dict['rgb_vis'] = rgb_vis
        ret_dict['rgb_w0'] = rgb_w0
        ret_dict['rgb_w1'] = rgb_w1
        ret_dict['rgb_gate'] = rgb_gate
