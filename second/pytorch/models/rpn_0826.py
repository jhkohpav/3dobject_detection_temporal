...
class RPN_FUSION(nn.Module):
    ...
    def __init__(self,
        ...
        self.bev_gate = BasicGate(n_feats)
        self.crop_gate = BasicGate(n_feats)
        # self.idx_offset = Variable(torch.zeros([35200,2]).cuda())
        # self.idx_offset = Variable(0.01*torch.randn([35200,2], requires_grad=True).cuda())
        # self.idx_offset = 0.01*torch.randn([35200,2],requires_grad=True).cuda()

        # self.idx_linear = Sequential(
        #     nn.Linear(1024, 512),
        #     nn.ReLU(),
        #     nn.Linear(512, 128),
        #     nn.ReLU(),
        ...
        #     # nn.Sigmoid()
        # )
        # self.idx_offset_conv = nn.Conv2d(1, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, f_view, idxs_norm,idxs_norm_ori, bev=None):
        ...
        for b_idx in range(4):
            ...
            idxs[:,0] = idxs[:,0]/70.4*176
            idxs[:,1] = (idxs[:,1]+40)/80*200
        ...
        layer_name = ['depth', 'bev', 'fused_rgb']
