...
sys.path.append('/home/jhyoo/jhyoo/3DOD/new_15_second/second.pytorch/second/Rotated_ROIAlign')
...
class VoxelNet(nn.Module):
    def __init__(self,
        ...
        if use_sparse_rpn: # don't use this. just for fun.
            ...
            if 'FUSION' in rpn_class_name:
                rpn_class = rpn_class_dict["RPN_FUSION"]
    ...
    def forward(self, example):
        ...
        with torch.no_grad():
            ...
            if 'FUSION' in self.rpn_class_name:
                preds_dict = self.rpn(spatial_features, f_view, idxs_norm)
            else:
        ...
        pooler_rotated1 = ROIAlignRotated((6,6), spatial_scale = (1.), sampling_ratio = 0)
        ...
        if 'FUSION' in self.rpn_class_name:
            ...
            pooler_rotated2 = ROIAlignRotated((6,6), spatial_scale = (1.), sampling_ratio = 0)
        ...
        # second_box_preds = batch_first_box_preds
        # second_cls_preds = batch_first_cls_preds.view(batch_size_dev, each_size[0],1)
        ...
        if self.training:
            ...
            if self._use_iou_branch or self._use_iou_param_partaa:
                ...
                for i in range(batch_size_dev):
                    ...
                    box_preds_decode = self._box_coder.decode_torch(box_preds_[i][pos_idx],
                                                                    batch_anchors_[i][pos_idx])
                    reg_targets_decode = self._box_coder.decode_torch(reg_targets_[i][pos_idx],
                                                                      batch_anchors_[i][pos_idx])
                    # compute overlaps between the anchors and the targets
                    preds_by_targ_iou = self.similarity_fn_wrapper(similarity_fn,
                                                              box_preds_decode, reg_targets_decode)
            ...
            res = self.predict_v2(example, preds_dict, second_anchors)
    ...
    def predict_v2(self, example, preds_dict, batch_anchors):
        t = time.time()
